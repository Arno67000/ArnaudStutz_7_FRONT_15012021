...
export class TweetService {
  ...
  getAll() {
    ...
    this.httpClient
      .get<any[]>('http://localhost:8000/tweets', { headers: headers })
      .subscribe(
        (data) => {
          this.tweets = data;
          this.emitTweet();
  ...
  postTweet(tweet: Tweet) {
    const headers = this.getHeaders();
    this.httpClient
      .post<Tweet>('http://localhost:8000/tweets', tweet, { headers: headers })
      .subscribe(
        (tweet) => {
  ...
  deleteTweet(tweetId: string) {
    const headers = this.getHeaders();
    this.httpClient
      .delete('http://localhost:8000/tweets/' + tweetId, { headers: headers })
      .subscribe(
        (obj: any) => {
  ...
  modifyTweet(tweetId: string, tweet: Tweet) {
    const headers = this.getHeaders();
    this.httpClient
      .put<Tweet>('http://localhost:8000/tweets/' + tweetId, tweet, {
        headers: headers,
      })
      .subscribe(
        (newTweet) => {
